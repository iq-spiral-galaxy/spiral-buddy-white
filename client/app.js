...
const DEFAULT_VAULT_SUBDIR = "spiral-buddy-white";
...
function stripDisplayOrderPrefix(value) {
  const original = String(value ?? "").trim();
  const stripped = original
    .replace(/^\s*(?:chapter|챕터|ch)\.?\s*\d+\s*[-_.:)\]·–—]*\s*/i, "")
    .replace(
      /^\s*(?:(?:0\d{1,2})(?:\s*[-_.:)\]·–—]\s*|\s+)|(?:[1-9]\d?)\s*[-_.:)\]·–—]\s*)/,
      "",
    )
    .trim();
...
function displayRepoName(name) {
  ...
  // 구분 기호를 공백으로 바꾸기 전에 순번을 걷어내야 `10-remote`와
  // 실제 제목 `12 Factor App`을 안전하게 구분할 수 있다.
  s = stripDisplayOrderPrefix(s);
  s = s.replace(/-/g, " ").trim();
      ...
      return `
        <div class="ws-row ${isActive ? "active" : ""}">
          <div class="ws-row-main">
            ...
            <div class="ws-row-vaultsub">노트: <code>vault/${escapeHtml(w.vaultSubDir ?? DEFAULT_VAULT_SUBDIR)}/</code></div>
          </div>
...
function openRemoveWorkspaceModal(ws) {
  ...
  document.getElementById("remove-ws-notes-path").innerHTML = `<code>${escapeHtml(_settingsCache.vaultPath ?? "")}/${escapeHtml(ws.vaultSubDir ?? DEFAULT_VAULT_SUBDIR)}</code> 이 옆 폴더로 이동(보관)됩니다.`;
...
function renderCuratedPresets() {
  ...
  grid.innerHTML = presets
    .map((p) => {
      ...
      return `
        <button class="curated-preset-card${p.recommended ? " recommended" : ""}${p.heavy ? " heavy" : ""}" data-preset="${escapeAttr(p.id)}" type="button" style="--preset-tone: ${escapeAttr(p.color ?? "#d63384")}" ${!_curatedState.parentDir || _curatedState.busy ? "disabled" : ""}>
    ...
    .map((d) => {
      ...
      const btnLabel = isAllDone
        ...
        : isPartial
          ...
          : `받기 (${repos.length})`;
      return `
        <div class="curated-domain-row ${isAllDone ? "all-done" : ""} ${isPartial ? "partial" : ""}" style="--domain-tone: ${escapeAttr(d.color ?? "#d63384")}">
...
// 스트리밍 핸들(createStreamHandle/abortStreams/pumpStream 등)은 ./stream.js로 이동.
...
async function runLookup(query, depth, opts = {}) {
  ...
  if (rawSource?.trim() && saveButton) {
    ...
    _lookupConceptDrafts.set(card, {
      ...
      content: rawSource,
      userQuestion,
      // API의 depth는 lookup 응답 길이가 아니라 현재 학습 나선의 정수 depth다.
