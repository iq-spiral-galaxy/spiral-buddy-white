...
<p align="center">
  <img alt="release" src="https://img.shields.io/badge/release-v0.2.2-ec4899?style=flat-square">
...
- 🌉 **설명적 간극 인덱스** — 사이드바 *설명적 간극* 버튼. 모든 노트에서 "메커니즘이 1인칭 경험을 다 설명 못 한 지점"만 뽑아 **7레이어별로** 모아 보여줍니다. 학습이 쌓일수록 두꺼워지는 *"환원이 멈춘 경계들의 지도"* (검색·옵시디언 바로열기). White 철학의 핵심 산출물.
- 🔁 **원리 인덱스** — 사이드바 *원리* 버튼. 표상·예측·통합·자기참조·체화·창발 6원리가 **뉴런부터 자아까지 어느 레이어에서 반복되는지** 가로질러 모아 봅니다 (간극 인덱스의 짝).
- ✨ **원리 횡단 회수** — Synthesis(L6) 챕터에 들어가면 버디가 *다른 레이어에서 같은 원리를 만났던 과거 노트*를 자동으로 들고 시작해 "그때 그 구조랑 같지 않아?"라고 잇습니다.
- 🔭 **챕터 미리보기 — 마주칠 간극** — 챕터 진입 전 미리보기 카드가 요약·핵심 질문과 함께 *"이 챕터에서 마주칠 핵심 설명적 간극/직관의 함정"*을 한 줄 예고합니다.
- 🧭 **간극-인지 추천 + 종료-전 가드** — "다음에 뭘 볼까" 추천이 노트별 간극 짚음 여부를 읽어 *간극이 안 닫힌 챕터를 한 층 더* 권하고, 간극을 한 번도 안 짚고 세션을 끝내려 하면 저장 전에 한 턴 권합니다.
