...
describe("vault: listSpiralNotes", () => {
  ...
  test("writeNewNote writes through an already-loaded note cache", async () => {
    ...
    const filePath = await writeNewNote(
      vault,
      baseNewNote({ topic: "Fresh", depth: 1 }),
    );
    // 디렉터리를 옮겨 disk rescan이면 빈 목록이 되게 한다. write-through
    // cache라면 방금 저장한 metadata가 즉시 남아 있어야 한다.
    const spiralRoot = path.dirname(filePath);
    await fs.rename(spiralRoot, `${spiralRoot}-moved`);
    const notes = await listSpiralNotes(vault);
    assert.equal(notes.length, 1);
...
describe("roadmap: discoverRoadmaps + loadRoadmapChapters fixture", () => {
  ...
  test("exact loaders block traversal, ignored dirs, deep roadmaps and outside symlinks", async () => {
    const root = await mkTmp("rmsafe");
    const repo = path.join(root, "safe-rm");
    const outside = await mkTmp("rmoutside");
    await fs.mkdir(path.join(repo, ".private"), { recursive: true });
    ...
    await fs.writeFile(path.join(repo, ".private", "secret.md"), "# secret\n");
    await fs.writeFile(path.join(repo, "node_modules", "dep.md"), "# dep\n");
    ...
    const hidden = path.join(root, ".hidden-rm");
    const dependency = path.join(root, "node_modules", "package-rm");
    const tooDeep = path.join(root, "a", "b", "c", "d", "e", "f", "g");
    for (const dir of [hidden, dependency, tooDeep]) {
    ...
    assert.equal(
      await resolveLocalRoadmapExact(root, "node_modules/package-rm"),
      null,
    );
    assert.equal(
      await resolveLocalRoadmapExact(root, "a/b/c/d/e/f/g"),
      null,
    );
    ...
    assert.equal(await loadRoadmapChapter(roadmap!, "outside.md"), null);
    assert.equal(await loadRoadmapChapter(roadmap!, ".private/secret.md"), null);
    assert.equal(await loadRoadmapChapter(roadmap!, "node_modules/dep.md"), null);
  });
});
